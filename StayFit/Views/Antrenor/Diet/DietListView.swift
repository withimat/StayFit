        ...
        NavigationView {
                        ...
                        Section(header: Text(key).font(.headline).padding(.vertical)) {
                            ...
                            ForEach(meals, id: \.id) { diet in
                                NavigationLink(destination: DietMealDetailView(meal: diet).navigationBarBackButtonHidden(true)
                                    ...
                                    DietMealRowView(meal: diet)
                                       {
                                        viewModel.deleteDietMeal(by: diet.id)
                                    }
                                }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    NavigationLink(destination: AddDietMealView(viewModel: viewModel).navigationBarBackButtonHidden(true)) {
...
#Preview {
    DietListView( workoutdays: DietDays(id: 1, title: "Bacak", dayOfWeek: 0, isCompleted: false))
}
